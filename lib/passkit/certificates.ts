...
export const checkCertificateSetup = async (): Promise<{
  ...
  try {
    ...
    if (!config.certificatePath || config.certificatePath.includes('certificates/pass_certificate.pem')) {
      issues.push('Pass certificate path not configured. Set PASSKIT_CERTIFICATE_PATH environment variable.')
    }

    if (!config.keyPath || config.keyPath.includes('certificates/pass_private_key.pem')) {
      issues.push('Private key path not configured. Set PASSKIT_PRIVATE_KEY_PATH environment variable.')
    }

    if (!config.wwdrCertificatePath || config.wwdrCertificatePath.includes('certificates/wwdr_certificate.pem')) {
