...
export default function MyStickersPage() {
  ...
  const fetchCodes = async (filter: StatusFilter) => {
    try {
      ...
      const response = await fetch(`/api/my-stickers?${params.toString()}`)
    ...
    return (
      <Container maxW="6xl" mx="auto" py={8}>
        <Box textAlign="center" py={16}>
          <p className={css({ color: 'fg.muted', fontSize: 'lg' })}>Loading your stickers...</p>
        </Box>
      </Container>
    )
  }
      ...
      <Flex justify="space-between" align="center" mb={8}>
        <Box>
          ...
          <p className={css({ fontSize: 'sm', color: 'fg.muted' })}>
            Manage your purchased QR code stickers and view their assignment history
          </p>
        </Box>
        <button
      ...
      {codes.length === 0 ? (
        <Box textAlign="center" py={16}>
          ...
          <p className={css({ color: 'fg.muted', fontSize: 'lg', mb: 4 })}>
            {statusFilter === 'all'
              ? "You don't have any QR stickers yet."
              : `No ${statusFilter === 'owned_unassigned' ? 'available' : statusFilter} stickers.`
            }
          </p>
          {statusFilter === 'all' && (
            <button
