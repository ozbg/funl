...
interface CanvasProps {
  ...
  onElementAdd: (element: Omit<LayoutElement, 'id'>) => void
...
export default function Canvas({
  ...
  const handleElementMove = (id: string, deltaX: number, deltaY: number) => {
    ...
    const newX = Math.max(0, Math.min(90, element.position.x + (deltaX / canvasWidth) * 100))
    const newY = Math.max(0, Math.min(90, element.position.y + (deltaY / canvasHeight) * 100))
